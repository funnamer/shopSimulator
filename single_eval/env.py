...
class ShopEnv:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"ShopEnv initialized, base_url: {self.base_url}, if_persona: {self.if_persona}"
        )
    ...
    def interact(self, action_text: str) -> Dict[str, Any]:
        ...
        if self.env_idx is None:
            raise ValueError(
                "Environment not initialized, please call reset() to initialize"
            )
        ...
        if self.if_persona:
            parts = result["instruction"].split("[SEP]", 2)
            if len(parts) >= 3:
                result["instruction"] = f"{parts[0]}[SEP] {self.instruction} [SEP]{parts[2]}"
